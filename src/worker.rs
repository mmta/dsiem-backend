...
pub async fn start_worker(
    ...
    while let Some(message) = subscription.next().await {
        if let Ok(v) = str::from_utf8(&message.payload) {
            ...
            if res.is_err() {
                error!("cannot send event {}: {}, skipping it", e.event_id, res.unwrap_err());
            } else {
                debug!("event {} broadcasted", e.event_id);
            }
        } else {
...
async fn download_config_files(
    ...
    for f in files
        ...
        let mut local = File::create(conf_dir.clone() + "/" + &f.filename)?;
        local.write_all(content.as_bytes())?;
    }
